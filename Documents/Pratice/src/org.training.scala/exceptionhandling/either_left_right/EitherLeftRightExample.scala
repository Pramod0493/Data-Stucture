...
object EitherLeftRightExample extends App {
  ...
  def divideXByY(x: Int, y: Int): Either[String, Int] = {
    if (y == 0) Left("divide by zero is not possible")
    else Right(x / y)
  }

  /*def divideXByY(x: Int, y: Int): Either[(String,Throwable), Int] = {
    try {
      Right(x / y)
    }
    catch {
      case e => Left(("divide is not possbile because of this exception", e))
    }
  }*/
